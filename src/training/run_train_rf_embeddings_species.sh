#!/bin/bash
#SBATCH --job-name=train-rf-emb-species
#SBATCH --nodes=1
...
OUT_DIR="$RESULTS_DIR/final_model_species"
mkdir -p "$OUT_DIR"
cd "$WORK_DIR"

python src/training/train_rf_embeddings_species.py \
  --dataset_path "$DATASET_PATH" \
  --config all \
  --output_dir "$OUT_DIR"

echo "Done: species metrics & artefacts saved under $OUT_DIR"
