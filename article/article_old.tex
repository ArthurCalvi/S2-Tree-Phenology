...
In addition, we benchmark a 64-band foundation-model embedding computed from satellite imagery (AlphaEarth; \citealp{AlphaEarth2025}) as a generic representation, comparing its performance against our 14 interpretable harmonic features under identical eco-region folds and sampling weights. This neutral comparison clarifies trade-offs between generic embeddings and targeted, physics-informed features for national phenology mapping.
...
% Swap-in sentences depending on outcome (keep only one in final):
% If embeddings outperform harmonics nationally or in key regions:
% \textit{Preliminary results indicate that AlphaEarth embeddings outperform the harmonic baseline at national scale and in challenging eco-regions (e.g., Mediterranean, Corsica), suggesting that generic representations can capture phenological separability effectively without hand-crafted features.}
% If harmonics match or beat embeddings:
% \textit{Preliminary results show that targeted harmonic features match or exceed the performance of generic embeddings while remaining more interpretable and computationally efficient, reinforcing the value of physics-informed temporal descriptors for phenology mapping.}
